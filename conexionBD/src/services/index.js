...
const LIMPIAR_SORTEO_PERSONALIZADO_QUERY= "TRUNCATE TABLE sorteo_personalizado";
...
const getUserIdByName= (req, res) => {
  connection.query(`SELECT id FROM user WHERE nombre = '${req.params.nombre}'`,(err, result) => {
    if (err) {
        console.log(err);
    } else {
...
const getSorteo = (req, res) => {
    connection.query(`SELECT MAX(id_sorteo) AS id_sorteo, nombre_sorteo, minimo_participantes FROM sorteo_personalizado GROUP BY nombre_sorteo, minimo_participantes`,(err, result) => {
        if (err) {
            console.log(err);
        } else {
            res.status(200).send(result);
...
module.exports = {getSorteo,getGanador, addUser,
     crearSorteo, getUsers, getUserIdByName, dataToObject,
     qrGenerate};